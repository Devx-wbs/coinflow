...
<style>
    ...
    .plan-title { font-size: 1.5rem; font-weight: 700; margin-bottom: 11px; letter-spacing: 1px; }
    .plan-details, .info-label { margin-bottom: 8px; }
    .badge-active { background: #38d993; color: #fff; padding: 5px 18px; border-radius: 12px; font-size: 14px; }
    .license-row { display: flex; gap: 10px; align-items: center; margin: 18px 0 16px 0; }
    .copy-btn, .download-btn { border-radius: 7px; font-size: 14px; margin-right: 7px; padding: 7px 14px;}
    .copy-btn { background: #6366f1; color: #fff; border: none; transition: background 0.2s;}
    .copy-btn:hover { background: #4338ca;}
    .download-btn { background: #0ea5e9; color: #fff; border: none; transition: background 0.2s;}
    .download-btn:hover { background: #0369a1;}
    .cancel-btn { background: #ef4444; color: #fff; border-radius: 7px; border: none; padding: 9px 22px; margin-top: 11px; font-size: 16px; font-weight: 600; transition: background 0.2s;}
    .cancel-btn:hover { background: #dc2626;}
    .order-section { padding-top: 17px; }
    .order-card { background: #f3f5fa; border-radius: 13px; box-shadow: 0 2px 12px rgba(0,0,0,0.035); margin-bottom: 26px; padding: 17px 28px; 
        display: flex; justify-content: space-between; align-items: flex-start; }
    .order-info { display: flex; flex-direction: column; gap: 7px; }
    .order-pro  { color: #f59e42; font-size: 16px; font-weight: 500;}
    .order-free { color: #06b6d4; font-size: 16px; font-weight: 500;}
    .order-paid { font-weight: bold; color: #0e7490;}
    .order-status { font-size: 15px; margin-left: 8px;}
    .text-warning { color: #f59e42; font-weight: 700;}
    .status-active { color: #22c55e; font-weight: 700;}
    .custom-alert { font-weight: bold; border-radius: 11px; margin-bottom: 24px;}
    ...
    @if($subscription)
      ...
      @if($license)
        <div class="license-row">
          ...
          <a href="{{ route('download.zip') }}" class="download-btn" >Download Zip</a>
        </div>
        <script>
          function copyLicenseKey() {
            ...
            navigator.clipboard.writeText(input.value)
              .then(() => { alert("✅ License key copied: " + input.value); })
